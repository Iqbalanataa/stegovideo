...
# As we know a video is a collection of frames, where each frame is a picture.
...
def encode_string(input_string, root="./temp/"):
    ...
    for i in range(len(split_string_list)):
        ...
        secret_enc.save(f_name)  # Saved the frames after hiding the strings.
...
# This function would extract audio from the video so as to stitch them back later.
...
if __name__ == "__main__":
    while True:
        print("1. Hide a message in a video\n2. Reveal the secret from a video\n")
        print("Any other value to exit\n")
        choice = input("Enter your choice: ")
        if choice == '1':
            f_name = input("Enter the name of the video file with extension: ")
            input_main(f_name)
        elif choice == '2':
            decode_string(input("Enter the name of the video with extension: "))
        else:
            break
